...
@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-2 text-gray-800">Edit User Keuangan</h1>
        <p class="mb-4">Silakan edit data user keuangan berikut.</p>
...
@section('scripts')
    <script>
        document.getElementById('formEditKeuangan').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            try {
                const response = await fetch(`http://localhost:3000/api/users/keuangan/${id}`, {
                    method: 'PUT',
                    headers: {
                ...
                if (response.ok) {
                    window.location.href = '/admin/tim-keuangan?success=User berhasil diupdate';
                } else if (
                    result.message &&
