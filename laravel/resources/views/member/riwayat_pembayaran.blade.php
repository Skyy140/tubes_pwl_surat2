...
@section('content')
    ...
    <main class="main py-5">
        <div class="container">
            <h2 class="mb-4 fw-bold">Riwayat Pembayaran Saya</h2>
    ...
    <script>
        async function loadEventsSaya() {
            ...
            try {
                const res = await fetch(`http://localhost:3000/api/events/riwayat-pembayaran/user/${userId}`, {
                ...
                const rows = data.map((registrasi, index) => {
                    ...
                    let aksiButton = `<a href="/event-saya/${event.idevents}" class="btn btn-sm btn-primary me-1">Detail</a>`;

                    if (status === 'menunggu') {
                        aksiButton += `<a href="/event-saya/update-bukti/${registrasi.idregistrations}" class="btn btn-sm btn-warning">Update</a>`;
                    }
